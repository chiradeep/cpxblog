...
class DockerInterface:
    ...
    def get_service_members(self, svc_label_key, svc_label_value):
        ...
        ips = [c['NetworkSettings']['Networks'].values()[0]['IPAddress']
               for c in containers]
        ports = [c['Ports'][0]['PrivatePort'] for c in containers]

        # return list of ip, port [('172.22.0.2', 80), ('172.22.0.3', 80)]
    ...
    def get_service_url(self, svc_label_key, svc_label_value, svc_url_key):
        ...
        # assume that all values in the list are identical
        return label_values[0]

    def get_ns_port(self, container_name):
        ns = self.client.containers(filters={'name': container_name})
        port = [port['PublicPort'] for port in ns[0]['Ports']
